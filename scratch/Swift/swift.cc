    ...
    m_delayedAckReserved (false), m_initialized (false)
{
  ...
  m_alpha = 4 / ((1./sqrt(0.1))-(1./sqrt(100000)));
  m_beta = -1 * m_alpha / sqrt(100000);
}
...
uint32_t Swift::clamp(uint32_t x, uint32_t minVal, uint32_t maxVal) {
...
void Swift::UpdateTargetDelay(Ptr<TcpSocketState> tcb) {
  ...
  m_targetDelay = 1050000
                * ((1 + std::max(0.0, std::min((m_alpha / std::sqrt((double)tcb->m_cWnd) + m_beta), 4.0))));

  tcb->m_target_delay = m_targetDelay;
}
...
void Swift::IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked) {
  ...
  double delay = (double) tcb->m_lastRtt.Get().GetNanoSeconds();
  // double delay = (double) tcb->m_maxDelay;
  if(delay < m_targetDelay) {
    if(tcb->m_cWnd.Get() > 1) {
      tcb->m_cWnd = tcb->m_cWnd + (1. / tcb->m_cWnd) * segmentsAcked;
    }
    else {
      tcb->m_cWnd = tcb->m_cWnd + (1 * segmentsAcked);
    }
  }
  else {

    if(m_canDecrease) {
      tcb->m_cWnd = std::max(1 - (0.5 * (delay - m_targetDelay) / delay),
                            0.5) * tcb->m_cWnd;
    }

    // std::cout << "Decreased new cwnd:" << tcb->m_cWnd << std::endl;
  ...
  if(tcb->m_congState == TcpSocketState::CA_LOSS) {
    ...
    if(m_retransmitCount >= m_maxRetries) {
      ...
      if (m_canDecrease) {
        tcb->m_cWnd = (1 - m_maxDecrease) * tcb->m_cWnd; // where m_maxDecrease < 1.0
      }
    }
  }
...
void Swift::PktsAcked (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time &rtt) {
  ...
  if ((Simulator::Now() - m_lastDecrease) >= tcb->m_lastRtt) {
      m_canDecrease = true;
  } else {
      m_canDecrease = false;
  }

  // uint64_t maxDelayNs = tcb->m_maxDelay;

  // std::cout << "Max delay: " << maxDelayNs << std::endl;
